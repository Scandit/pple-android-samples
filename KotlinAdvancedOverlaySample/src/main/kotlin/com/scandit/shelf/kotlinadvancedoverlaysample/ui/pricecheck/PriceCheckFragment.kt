...
import com.scandit.shelf.kotlinadvancedoverlaysample.ui.base.CameraPermissionFragment
import com.scandit.shelf.kotlinadvancedoverlaysample.ui.login.LoginFragment
...
import com.scandit.shelf.sdk.price.ui.PriceCheckAdvancedOverlayListener
...
class PriceCheckFragment : CameraPermissionFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val captureView = rootView.findViewById<CaptureView>(R.id.capture_view)

        collectFlows()

        viewModel.initPriceCheck(
            captureView,
            createAdvancedPriceCheckOverlay(),
    ...
    private fun createAdvancedPriceCheckOverlay(): AdvancedPriceCheckOverlay =
        AdvancedPriceCheckOverlay(object : PriceCheckAdvancedOverlayListener {
            override fun viewForPriceCheckResult(priceCheckResult: PriceCheckResult): View {
                val statusImageResource = getStatusImageResource(priceCheckResult)
                // Return the view that needs to be displayed.
                return ImageView(requireContext()).apply {
                    setImageResource(statusImageResource)
                }
            }
        })

    private fun getStatusImageResource(priceCheckResult: PriceCheckResult) =
        when (priceCheckResult.correctPrice) {
            null -> R.drawable.ic_price_check_unknown_product
            priceCheckResult.capturedPrice -> R.drawable.ic_price_check_correct_price
            else -> R.drawable.ic_price_check_wrong_price
        }
