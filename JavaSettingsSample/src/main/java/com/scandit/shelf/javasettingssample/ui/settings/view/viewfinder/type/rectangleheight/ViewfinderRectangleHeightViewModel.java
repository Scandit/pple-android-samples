...
public class ViewfinderRectangleHeightViewModel  extends ViewfinderTypeViewModel {

    private final ViewfinderRepository repository = ViewfinderRepository.getCurrentSettings();


    void valueChanged(float newValue) {
        MeasureUnit currentMeasureUnit = repository.getRectangularViewfinderHeight().getUnit();
        repository.setRectangularViewfinderHeight(
                new FloatWithUnit(newValue, currentMeasureUnit)
        );
        updateViewfinder();
    }

    void measureChanged(MeasureUnit measureUnit) {
        float currentValue = repository.getRectangularViewfinderHeight().getValue();
        repository.setRectangularViewfinderHeight(
                new FloatWithUnit(currentValue, measureUnit)
        );
        updateViewfinder();
    }

    FloatWithUnit getCurrentHeight() {
        return repository.getRectangularViewfinderHeight();
    }
}
